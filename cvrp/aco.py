...
class ACO():
    ...
    def update_pheronome(self, paths, costs):
        ...
        '''
        self.pheromone = self.pheromone * self.decay    # 对信息素进行衰减
        
        if self.elitist:    # 如果是精英选择策略
            best_cost, best_idx = costs.min(dim=0)
    ...
    def gen_path(self, require_prob=False):
        ...
        visit_mask = self.update_visit_mask(visit_mask, actions)    # 对已经访问的节点进行掩码
